...
def _load_document(path: str) -> fitz.Document:
    ...
    Images are embedded as full-page PDF content so we can annotate them.
    """
    p = Path(path)
    if not p.exists():
        # Return a blank A4 doc as fallback
        doc = fitz.open()
    ...
    if _is_image(path):
        ...
        # Create a page matching the image aspect ratio (max A4-ish width)
        max_w = 595
        scale = max_w / img_rect.width if img_rect.width > max_w else 1.0
        page_w = img_rect.width * scale
        page_h = img_rect.height * scale

        page = img_doc.new_page(width=page_w, height=page_h)

        # Render the image as page content
...
def _draw_tick(page: fitz.Page, cx: float, cy: float, size: float, color: tuple):
    ...
    # Checkmark path: down-left to bottom, then up-right
    shape.draw_line(
    ...
    shape.finish(color=color, width=max(2.5, size * 0.12), closePath=False,
...
def _draw_circled_score(page: fitz.Page, cx: float, cy: float, radius: float,
    ...
    shape.finish(color=color, fill=C.WHITE, width=2.2)
    shape.commit()

    # Score text
    score_text = f"{score:g}/{max_marks:g}"
    fs = min(14, radius * 0.7)
...
def _draw_feedback_strip(page: fitz.Page, x: float, y: float, width: float,
    ...
    strip_h = padding * 2 + len(lines) * line_h

    # Background
    shape = page.new_shape()
    rect = fitz.Rect(x, y, x + width, y + strip_h)
    shape.draw_rect(rect)
    shape.finish(color=color, fill=bg, width=0.8)
    shape.commit()

    # Left accent bar
    accent = page.new_shape()
    accent.draw_rect(fitz.Rect(x, y, x + 3, y + strip_h))
    ...
    text_y = y + padding + fs
    for line in lines:
        writer.append(fitz.Point(x + padding + 4, text_y), line,
                      fontsize=fs, font=fitz.Font("helv"))
        text_y += line_h
...
def _draw_concept_row(page: fitz.Page, x: float, y: float, max_width: float,
    ...
    for concept in concepts[:6]:
        ...
        # Pill text
        writer = fitz.TextWriter(page.rect)
        writer.append(fitz.Point(cx + 4, row_y + pill_h - 4), label,
                      fontsize=fs, font=fitz.Font("helv"))
        writer.write_text(page, color=color)

        cx += pill_w + gap

    return row_y + pill_h + 4
...
def _annotate_page(page: fitz.Page, questions: list[QuestionAnnotation]):
    ...
    Layout per question (stacked vertically from top):
    ┌──────────────────────────────────────────────┐
    │ ✓/✗  Q1 — Correct               ┌──(4/5)──┐│
    │ ▌ Feedback strip with text       └─────────┘│
    │ [✓ concept] [✓ concept] [✗ missed]          │
    └──────────────────────────────────────────────┘
    """
    pw = page.rect.width
    ph = page.rect.height
    ...
    for q in questions:
        ...
        if is_correct:
            ...
            _draw_tick(page, mark_x, mark_y, mark_size * 0.8, C.AMBER)
            # Small tilde/wave to indicate "almost"
        ...
        bar_shape.draw_rect(fitz.Rect(margin_l - 6, block_top, margin_l - 2, block_top + bar_h))
        ...
        sep.finish(color=C.LIGHT_GRAY, width=0.5)
        sep.commit()
        y += 8
...
def _build_summary_page(doc: fitz.Document, req: StudentAnnotationRequest):
    ...
    # ── Dark header ──
    _fill_rect(page, 0, 0, pw, 58, C.HEADER_BG)
    _text_centered(page, cx, 36, "Graded Answer Sheet — Summary", 15, C.WHITE)
    ...
    for label, value in info:
        _text(page, 40, y, f"{label}:", 9, C.MID_GRAY)
        _text(page, 100, y, value, 10, C.BLACK)
        y += 18
    ...
    _fill_rect(page, 35, y, pw - 70, 50, hero_bg, border_color=hero_color, border_w=1.2)
    _text(page, 50, y + 22, "Total Score:", 11, C.DARK_GRAY)
    score_str = f"{req.total_score:g} / {req.max_total:g}   ({ratio * 100:.0f}%)"
    _text(page, 145, y + 22, score_str, 14, hero_color)

    # Large grade mark
    if ratio >= 0.7:
        _draw_tick(page, pw - 75, y + 25, 24, hero_color)
    elif ratio >= 0.4:
        _draw_tick(page, pw - 75, y + 25, 20, hero_color)
    else:
        _draw_cross(page, pw - 75, y + 25, 20, hero_color)

    y += 65

    # ── Table ──
    col_x = [40, 90, 145, 200, 280, 370]
    headers = ["Q.No", "Score", "Max", "Band", "Feedback", "Concepts"]

    # Header row
    _fill_rect(page, 35, y - 2, pw - 70, 20, C.LIGHT_GRAY, border_color=C.MID_GRAY, border_w=0.5)
    for i, h in enumerate(headers):
        _text(page, col_x[i], y + 12, h, 8, C.DARK_GRAY)
    y += 24
    ...
    for idx, q in enumerate(sorted_qs):
        ...
        if idx % 2 == 0:
            _fill_rect(page, 35, y - 2, pw - 70, 20, (0.98, 0.98, 0.99))

        # Color dot
        dot_shape = page.new_shape()
        dot_shape.draw_circle(fitz.Point(col_x[0] - 6, y + 7), 3)
        dot_shape.finish(color=color, fill=color, width=0)
        dot_shape.commit()

        _text(page, col_x[0], y + 12, q.q_number, 9, C.BLACK)
        _text(page, col_x[1], y + 12, f"{q.score:g}", 9, color)
        _text(page, col_x[2], y + 12, f"{q.max_marks:g}", 9, C.DARK_GRAY)
        _text(page, col_x[3], y + 12, _band_label(q.grade_band), 8, color)

        fb_short = (q.feedback[:40] + "…") if len(q.feedback) > 43 else q.feedback
        _text(page, col_x[4], y + 12, fb_short, 7, C.MID_GRAY)
        ...
        if matched or missed:
            ...
            _text(page, col_x[5], y + 12, concept_info, 7.5,
    ...
    _text_centered(page, cx, ph - 18, footer, 6.5, C.MID_GRAY)
...
def annotate_student_image(request: StudentAnnotationRequest) -> bytes:
    ...
    # Annotate page 0 with all questions
    if len(doc) > 0:
        _annotate_page(doc[0], request.questions)
