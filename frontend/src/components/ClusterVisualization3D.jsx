...
const BAND_COLORS = { excellent: '#22c55e', good: '#3b82f6', average: '#f59e0b', poor: '#ef4444' }
const BAND_LABELS = { excellent: 'Excellent', good: 'Good', average: 'Average', poor: 'Needs Improvement' }
const BAND_ORDER = ['excellent', 'good', 'average', 'poor']

function getBandHex(kind) { return BAND_COLORS[kind] || '#ef4444' }
...
function Sphere({ node, selected, hovered, onSelect, onHover, onLeave }) {
  const ref = useRef()
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial
        ...
        roughness={0.35} metalness={0.15} transparent opacity={selected ? 1 : 0.82}
      />
      {(hovered || selected) && (
...
function Lines({ nodes, band }) {
  const geo = useMemo(() => {
    ...
    const pts = []
    nodes.forEach((n) => { pts.push(n.pos[0], n.pos[1], n.pos[2], cx, cy, cz) })
...
function Camera() {
  ...
  useEffect(() => { camera.position.set(0, 5, 14); camera.lookAt(0, 0, 0) }, [camera])
  return null
}
...
function buildLayout(clusters) {
  const out = []
  const regionX = { excellent: -5, good: -1.5, average: 2, poor: 5.5 }
  ...
  for (const cl of clusters) {
    ...
    const maxMarks = first && first.score_ratio > 0 ? Math.round(first.score / first.score_ratio) : 5
    ...
    students.forEach((st, i) => {
      ...
      out.push({
        ...
        pos: [
          rx + Math.cos(angle) * r,
          ((st.score_ratio || 0) - 0.5) * 3 + ((h % 50) / 50 - 0.5) * 0.25,
          Math.sin(angle) * r + ((h % 100) / 100 - 0.5) * 0.4,
        ],
        color: hex,
  ...
  return (
    <div style={{
      ...
      <div style={{ marginTop: 14, display: 'flex', gap: 12, alignItems: 'center' }}>
        <div style={{ fontSize: 28, fontWeight: 800, color: node.color }}>{node.score}/{node.maxMarks}</div>
        <div>
          <div style={{
            display: 'inline-block', background: `${node.color}22`, color: node.color,
            border: `1px solid ${node.color}44`, padding: '2px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600,
...
function Scene({ nodes, bandGroups, bandLabels, selectedId, hoveredId, onSelect, onHover, onLeave }) {
  return (
    <>
      <Camera />
      ...
      {Object.entries(bandGroups).map(([b, g]) => <Lines key={b} nodes={g} band={b} />)}

      {nodes.map((n) => (
        <Sphere key={n.id} node={n} selected={selectedId === n.id} hovered={hoveredId === n.id}
          onSelect={() => onSelect(n.id)} onHover={() => onHover(n.id)} onLeave={onLeave} />
      ))}

      {bandLabels.map(({ band, pos, count }) => (
        <Html key={band} position={new THREE.Vector3(...pos)} center style={{ pointerEvents: 'none' }}>
          <div style={{ color: BAND_COLORS[band], fontSize: 13, fontWeight: 700, textAlign: 'center', textShadow: '0 2px 10px rgba(0,0,0,0.8)', opacity: 0.92 }}>
            {BAND_LABELS[band]}
            <div style={{ fontSize: 10, fontWeight: 400, opacity: 0.65 }}>{count} student{count !== 1 ? 's' : ''}</div>
          </div>
        </Html>
      ))}
      ...
      <OrbitControls enablePan enableZoom enableRotate minDistance={5} maxDistance={25} autoRotate autoRotateSpeed={0.3} />
    </>
  )
}
...
export default function ClusterVisualization3D({ clusters }) {
  const [selectedId, setSelectedId] = useState(null)
  const [hoveredId, setHoveredId] = useState(null)

  const nodes = useMemo(() => buildLayout(clusters || []), [clusters])
  const selectedNode = useMemo(() => nodes.find((n) => n.id === selectedId) || null, [nodes, selectedId])

  const bandGroups = useMemo(() => {
    const g = {}
    nodes.forEach((n) => { (g[n.band] = g[n.band] || []).push(n) })
    return g
  }, [nodes])

  const bandLabels = useMemo(() => {
    const rX = { excellent: -5, good: -1.5, average: 2, poor: 5.5 }
    return BAND_ORDER
      .filter((b) => bandGroups[b]?.length)
      .map((b) => {
        const g = bandGroups[b]
        const avgY = g.reduce((s, n) => s + n.pos[1], 0) / g.length
        return { band: b, pos: [rX[b], avgY + 2.8, 0], count: g.length }
      })
  }, [bandGroups])
  ...
  if (nodes.length === 0) {
    return <div style={{ height: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#888', fontSize: 14 }}>No students to visualize.</div>
  }
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: 540, borderRadius: 20, overflow: 'hidden' }}>
      ...
      >
        <Scene nodes={nodes} bandGroups={bandGroups} bandLabels={bandLabels}
      ...
      {!selectedId && !hoveredId && (
        <div style={{
          ...
          color: 'rgba(255,255,255,0.45)', fontSize: 11, background: 'rgba(0,0,0,0.45)',
          padding: '4px 14px', borderRadius: 20, pointerEvents: 'none', zIndex: 30,
        }}>
